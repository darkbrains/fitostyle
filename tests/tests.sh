...
# Start fitostyle Docker container
docker network create actions --driver bridge

docker run --network actions --name fitostyle-tests -p 8080:8080 \
  darkbrains/fitostyle:local &

sleep 5


# Test /api/healthz endpoint
HTTP_STATUS_HEALTHZ=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:8080/api/healthz -H "Content-Type: application/json")
...
# Test / endpoint
HTTP_STATUS_SLASH=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:8080/ -H "Content-Type: application/json")
...
# Test /en endpoint
HTTP_STATUS_EN=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:8080/en -H "Content-Type: application/json")
...
# HTTP_STATUS_SLASH=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:8080/hotels -H "Content-Type: application/json")
# if [ "$HTTP_STATUS_SLASH" -eq 302 ]; then
...
# HTTP_STATUS_HOTELS_EN=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:8080/hotels/en -H "Content-Type: application/json")
...
# HTTP_STATUS_HOTELS_RU=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:8080/hotels/ru -H "Content-Type: application/json")
...
# HTTP_STATUS_HOTELS_AM=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:8080/hotels/am -H "Content-Type: application/json")
...
# Test /empty
HTTP_STATUS_EMPTY=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:8080/empty -H "Content-Type: application/json")
...
docker stop fitostyle-tests
docker rm fitostyle-tests
